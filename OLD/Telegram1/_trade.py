...
def buy_or_sell(df):
    ...
    logger.info(f"Precio actual: {current_price}, {config['BINANCE_TRACE_INTERVAL']} BBSup: {current_upper:.4f} BBInf: {current_lower:.4f} DBBSup {percentage_current_upper:.2f}% DBBInf {percentage_current_lower:.2f}%") #=> {current_price_percentage:.2f}%
    return (current_price, current_upper, current_lower, current_price_percentage, percentage_current_upper, percentage_current_lower)
...
def bollinger_trade_logic():
    ...
    (current_price, current_upper, current_lower, current_price_percentage, percentage_current_upper, percentage_current_lower) = buy_or_sell(symbol_df)
    return (current_price, current_upper, current_lower, current_price_percentage, percentage_current_upper, percentage_current_lower)
