...
def mainloop():
    ...
    while True:
        ...
        (current_price, current_upper, current_lower,
         current_price_percentage, percentage_current_upper, percentage_current_lower) = bollinger_trade_logic()
        _message_caption = f"{config['BINANCE_TRACE_SYMBOL']}, precio: ${current_price:.4f}\n"
        _caption = f"{now_time} - Precio actual: {current_price:.4f}, Temporalidad: {config['BINANCE_TRACE_INTERVAL']}, BB Superior: {current_upper:.4f}, BB Inferior: {current_lower:.4f}, Ratio: {current_price_percentage:.2f}%"
        ...
        if percentage_current_upper >= 0.5: #Porcentaje por fuera de la banda de bolinger para short
        ...
        elif percentage_current_lower >= 0.5: #Porcentaje por fuera de la banda de bolinger para long
        ...
        if (this_message_type == "Sell" or this_message_type == "Buy") and \
           ((last_message_type != this_message_type) or msg_cnt > 90): #Cantidad de ciclos antes de repetir mensaje 90
