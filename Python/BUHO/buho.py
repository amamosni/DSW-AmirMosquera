...
def mainloop():
    ...
    while True:
        ...
        #print(f" 5 últimos a Long: {promedioA1:.2f}, {promedioA2:.2f}, {promedioA3:.2f}, {promedioA4:.2f}, {promedioA5:.2f}")
        #print(f"5 últimos a Short: {promedioB1:.2f}, {promedioB2:.2f}, {promedioB3:.2f}, {promedioB4:.2f}, {promedioB5:.2f}")
        #print(f" 3 últimos a Long: {promedioA1:.2f}, {promedioA2:.2f}, {promedioA3:.2f}")
        #print(f"3 últimos a Short: {promedioB1:.2f}, {promedioB2:.2f}, {promedioB3:.2f}")
        ...
        print(f"  A Long: {distance_block_abajo_promedio:.2f}")
        print(f" A Short: {distance_block_arriba_promedio:.2f}")
        #print(f"Precio: {current_price:.5}")

        if order_book_cnt >= 4: #Cantidad de ciclos antes de repetir mensaje
            # Determine Sell(Short) or Buy(Long) or Skip(Dejar)
        
            if distance_block_arriba_promedio <= float(config['OB_PRESION_SHORT']) and rsi >= float(config['RSI_SHORT']): #Porcentaje cerca de la order book para short
                _message = "🟥 SHORT en " + _message_caption + "\nPresión del: " + f"{distance_block_arriba_promedio:.2f}%, RSI {rsi:.2f}" + "\n🤖 Bot BUHO\n" + now_time
                this_message_type = "Sell"
            elif distance_block_abajo_promedio <= float(config['OB_PRESION_LONG']) and rsi <= float(config['RSI_LONG']): #Porcentaje cerca de la order book para long
                _message = "🟩 LONG en " + _message_caption + "\nPresión del: " + f"{distance_block_abajo_promedio:.2f}%, RSI {rsi:.2f}" + "\n🤖 Bot BUHO\n" + now_time
                this_message_type = "Buy"
            else:
                this_message_type = "Skip"

            # Revisar si entra en operación
            if (this_message_type == "Sell" or this_message_type == "Buy") and ((last_message_type != this_message_type) or order_book_cnt > 4):
